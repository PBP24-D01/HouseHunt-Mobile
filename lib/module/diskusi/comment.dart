  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      final sellerResponse = await request.get(
          'http://127.0.0.1:8000/diskusi/show_seller/${widget.id}');
      if (sellerResponse is List) {
          seller = sellerResponse
            .map((item) => ShowSeller.fromJson(item))
            .toList();
      }

      // Fetch comments
      final commentResponse = await request
          .get('http://127.0.0.1:8000/diskusi/show_comment/${widget.id}');
      if (commentResponse is List) {
        comments = commentResponse
            .map((item) => ShowComment.fromJson(item))
            .toList();
      }

      // Fetch replies for each comment
      for (var comment in comments) {
        final replyResponse = await request
            .get('http://127.0.0.1:8000/diskusi/show_reply/${comment.pk}');
        if (replyResponse is List) {
          replies[comment.pk] = replyResponse
              .map((item) => ShowReply.fromJson(item))
              .toList();
        }
      }
...
Future<void> submitComment() async {
    ...
    try {
      final response = await request.post(
        'http://127.0.0.1:8000/diskusi/add_comment_api/${widget.id}',
        {
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : SingleChildScrollView(
              child: Column(
                children: [
                  if(context.read<CookieRequest>().jsonData['is_buyer'] == true)
                  // Comment Input
                  Padding(
...
class _CommentCardState extends State<CommentCard> {
  ...
  void initState() {
    ...
    try {
      await request.post(
        'http://127.0.0.1:8000/diskusi/reply_api/${widget.comment.pk}',
        {
  ...
  Future<void> updateReply(String replyId) async {
    ...
    try {
      await request.post(
        'http://127.0.0.1:8000/diskusi/edit_reply_api/$replyId',
        {
  ...
  Future<void> deleteReply(String replyId) async {
    ...
    try {
      await request.post(
        'http://127.0.0.1:8000/diskusi/delete_reply_api/$replyId',
        {},
      );
      widget.refreshComments();
    } catch (e) {
  ...
  Future<void> updateComment() async {
    ...
    try {
      await request.post(
        'http://127.0.0.1:8000/diskusi/edit_comment_api/${widget.comment.pk}',
        {
  ...
  Future<void> deleteComment() async {
    ...
    try {
      await request.post(
        'http://127.0.0.1:8000/diskusi/delete_comment_api/${widget.comment.pk}',
        {},
      );
      widget.refreshComments();
    } catch (e) {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  if (context.read<CookieRequest>().jsonData['company_name'] == widget.companyName)  // Add this condition
                    Row(
                      children: [
                      IconButton(
                  ...
                  if (widget.comment.fields.name == context.read<CookieRequest>().jsonData['username'])
                    Row(
                      children: [
                        IconButton(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (reply.fields.name == context.read<CookieRequest>().jsonData['company_name'])
                            Row(
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      ...
                                      editReplyController.text = reply.fields.body;
                                    });
                                  },
                                ),
                                IconButton(
